...
class REST_Controller extends CI_Controller {
    ...
    protected $api_key =NULL; // Stores the API key (used insome libraries)
    ...
    /**
     *  Modified out by Serna for Phlare Project 
     *  we want to validate aginst the DB not 
     *  the config  file 
     *  07/23/2011
     * 
     */
	private function _check_login($username = '', $password = NULL)
	{
       if (empty($username))
		...
		//$valid_logins = & $this->config->item('rest_valid_logins');
         $valid_logins = $this->_get_valid_logins();
		...
		if ($password !== NULL AND $valid_logins[$username] != md5($password.$this->config->item('rest_valid_logins_salt')))
		{
			return FALSE;
		}
    ...
    private function _get_valid_logins()
    {
    
        if($this->config->item('rest_valid_logins_use_db'))
        {
            ...
            {
                $valid_logins[$row->username] = $row->password;
            }                   
	...
	private function _prepare_digest_auth()
	{
		...
		{
            log_message('DEBUG', 'API DIGEST AUTH FAILED');
